...
var _ = Describe("deploying a flask web app", func() {
	...
	Context("app has Pipfile.lock and no requirements.txt or runtime.txt", func() {
		BeforeEach(func() {
			app = cutlass.New(filepath.Join(testdata, "flask_python_3_pipenv"))
			app.SetEnv("BP_DEBUG", "1")
			PushAppAndConfirm(app)
		})

		PIt("deploys a pipenv app", func() {
		...
		Context("python 3", func() {
			Context("app is completely vendored", func() {
				BeforeEach(func() {
					...
					app = cutlass.New(filepath.Join(testdata, "flask_python_3_pipenv_vendored"))
					app.SetEnv("BP_DEBUG", "1")
				})
			...
			Context("app is missing a dependency", func() {
				BeforeEach(func() {
					...
					app = cutlass.New(filepath.Join(testdata, "flask_python_3_pipenv_vendored_incomplete"))
					app.SetEnv("BP_DEBUG", "1")
				})

				PIt("should work by downloading the missing dependency", func() {
					PushAppAndConfirm(app)
					Expect(app.GetBody("/")).To(ContainSubstring("Hello, World with pipenv!"))
				})
			})
	...
	Context("no Pipfile", func() {
		...
		PIt("deploys without downloading pipenv", func() {
			PushAppAndConfirm(app)
	...
	Context("When there is a requirements.txt and a Pipfile", func() {
		BeforeEach(func() {
			app = cutlass.New(filepath.Join(testdata, "pipfile_and_requirements"))
			app.Buildpacks = []string{"python_buildpack"}
			app.SetEnv("BP_DEBUG", "1")
		})
