...
var _ = Describe("running supply buildpacks before the python buildpack", func() {
	...
	Context("a simple app is pushed once", func() {
		BeforeEach(func() {
			...
			app = cutlass.New(filepath.Join(testdata, "fake_supply_python_app"))
		...
		PIt("finds the supplied dependency in the runtime container", func() {
	...
	Context("an app is pushed multiple times", func() {
		...
		BeforeEach(func() {
			...
			var err error
			tmpDir, err = cutlass.CopyFixture(filepath.Join(testdata, "flask_git_req"))
			Expect(err).To(BeNil())
		...
		PIt("pushes successfully both times", func() {
			app.Buildpacks = []string{
	...
	Context("the app uses miniconda", func() {
		BeforeEach(func() {
			...
			app = cutlass.New(filepath.Join(testdata, "miniconda_python_3"))
			app.Buildpacks = []string{
				"https://buildpacks.cloudfoundry.org/fixtures/supply-cache-new.zip",
				"python_buildpack",
			}
