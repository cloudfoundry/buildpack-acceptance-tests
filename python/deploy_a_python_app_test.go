...
var _ = Describe("CF Python Buildpack", func() {
	...
	Context("with an unsupported dependency", func() {
		BeforeEach(func() {
			app = cutlass.New(filepath.Join(testdata, "unsupported_version"))
		})

		PIt("displays a nice error messages and gracefully fails", func() {
	...
	PIt("deploy a web app with -e in requirements.txt", func() {
		app = cutlass.New(filepath.Join(testdata, "flask_git_req"))
	...
	PIt("deploy a web app with -r in requirements.txt", func() {
		app = cutlass.New(filepath.Join(testdata, "recursive_requirements"))
		PushAppAndConfirm(app)
		Expect(app.GetBody("/")).To(ContainSubstring("Hello, World!"))
	})

	PIt("deploy a web app that uses an nltk corpus", func() {
		app = cutlass.New(filepath.Join(testdata, "nltk_flask"))
		app.SetEnv("BP_DEBUG", "1")
		app.Memory = "256M"
		PushAppAndConfirm(app)
	...
	PIt("deploy a web app that uses an tkinter", func() {
		app = cutlass.New(filepath.Join(testdata, "tkinter"))
	...
	PIt("should not display the allow-all-external deprecation message", func() {
	...
	PIt("app has pre and post scripts", func() {
		app = cutlass.New(filepath.Join(testdata, "with_hooks"))
		PushAppAndConfirm(app)
	...
	Context("uncached buildpack", func() {
		...
		Context("pushing a Python 3 app with a runtime.txt", func() {
			Context("including flask", func() {
				BeforeEach(func() {
					app = cutlass.New(filepath.Join(testdata, "flask_python_3"))
					app.SetEnv("BP_DEBUG", "1")
				})
			...
			Context("including flask and no build isolation", func() {
				BeforeEach(func() {
					app = cutlass.New(filepath.Join(testdata, "no_build_isolation"))
					app.SetEnv("BP_DEBUG", "1")
				})
			...
			Context("including django with specified python version", func() {
				BeforeEach(func() {
					app = cutlass.New(filepath.Join(testdata, "django_python_3"))
					app.SetEnv("BP_DEBUG", "1")
				})
		...
		Context("pushing a Python app without the runtime.txt", func() {
			Context("", func() {
				...
				BeforeEach(func() {
					app = cutlass.New(filepath.Join(testdata, "flask"))
					for _, defaultDep := range manifest.DefaultVersions {
				...
				PIt("deploys with default Python version", func() {
					PushAppAndConfirm(app)
			...
			Context("including django but not specified Python version", func() {
				BeforeEach(func() {
					app = cutlass.New(filepath.Join(testdata, "django_web_app"))
				})
			...
			Context("including flask without a vendor directory", func() {
				BeforeEach(func() {
					app = cutlass.New(filepath.Join(testdata, "flask_not_vendored"))
					app.SetEnv("BP_DEBUG", "1")
				})
		...
		Context("with mercurial dependencies", func() {
			BeforeEach(func() {
				app = cutlass.New(filepath.Join(testdata, "mercurial"))
				app.SetEnv("BP_DEBUG", "1")
			})
	...
	PIt("sets gunicorn to send access logs to stdout by defualt", func() {
		app = cutlass.New(filepath.Join(testdata, "flask_latest_gunicorn"))
		PushAppAndConfirm(app)
